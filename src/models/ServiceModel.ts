...
export default class ServiceModel implements IServiceModel {
  ...
  async findAll(): Promise<IServices[]> {
  ...
  async insertService(data: TypeInsertService): Promise<InsertService> {
    ...
    const pieces = data.pieces.map(async (piece: Omit<IPieceServices, 'serviceId'>) => {
    ...
    const employeeServices = data.employeeServices.map(async ({ labor, employeeId, description }:
